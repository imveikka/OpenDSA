...
Here is a class declaration template for the BST:
...
``find`` calls private member function ``__findhelp``.
...
Member ``__findhelp`` has the desired form for this recursive
...
Note that, except for the last node in the path, ``__inserthelp``
...
duplicate in the left subtree. In this implementation, the BST does not allow duplicate keys. In this implementation, the BST does not allow duplicate keys.
...
The return value of the ``__deletemax`` method is the subtree of
the current node with the maximum-valued node in the subtree removed.
Similar to the ``__inserthelp`` method, each node on the path back to
the root has its right child node reassigned to the subtree
resulting from its call to the ``__deletemax`` method.

A useful companion method is ``__getmax`` which returns the maximum value in the subtree.

.. codeinclude:: Binary/BST
   :tag: getmax

Now we are ready for the ``__removehelp`` method.
...
its pointer set to :math:`R`'s child (similar to ``__deletemax``).
...
The cost for ``__findhelp`` and ``__inserthelp`` is the depth of
the node found or inserted.
The cost for ``__removehelp`` is the depth of the node being
